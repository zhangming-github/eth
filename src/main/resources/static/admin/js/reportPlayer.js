...
var reportPage = 1;
	...
	$.ajax({
		...
		success:function(result){
			...
			if(result==null){
				str += "<tr>" ;
				str += '<p style="text-align:center"><b>举报玩家列表为空</b></p>';               
				str += "</tr>";
			} else {
				str +=  '<table class="table table-hover">';
				str +=  '<caption><h4 style="text-align:center">举报玩家列表如下</h4></caption>';
				str +=  '<thead>';
				str +=  '<tr>';
				str +=  "<td width='80px' align='center'>序号</td>";
				str +=  "<td width='80px' align='center'>被举报人名字</td>";
				str +=  "<td width='80px' align='center'>违规内容</td>";
				...
				"<button id='shelve' class='btn btn-danger' name="+result[i].toplayerId+" value="+result[i].fromplayer+","+result[i].id+","+result[i].protectEyeTitle+" onclick='insCell(this)'>搁置</button></td>";
				str += "</tr>";
				str += "</tr>";
				numbe++;
				}
...
function insCell(th){
		...
		if(arr[2] == 0){
			layer.open({
				  ...
				  content: "<br>&nbsp&nbsp<b>玩家  :</b>&nbsp;<b>" + arr[0] +"</b><br><br>"
					+"&nbsp&nbsp<b>操作 :</b><button class='btn btn-danger' style='margin-left:5px;' id='banned'>封禁账号</button><br><br>"
					...
					+"<div class='input-group'  style='width:200px;' id='jinyan'><input type='text' id='duration' class='form-control' placeholder='区间5分钟~9999分钟'></div>"
					+"<div><button class='btn btn-primary' id='custom'>自定义时长禁言</button></div>"
			});
		} else {
			layer.open({
				  type: 1,
				  ...
				  content: "<br>&nbsp&nbsp<b>玩家  :</b>&nbsp;<b>" + arr[0] +"</b><br><br>"
					...
					+"<button class='btn btn-default' id='thirtyDay' style='margin-left:3px;'>30天</button>"
					+"<div class='input-group'  style='width:200px;' id='jinyan'><input type='text' id='duration' class='form-control' placeholder='区间5分钟~9999分钟'></div>"
					+"<div><button class='btn btn-warning' id='custom'>自定义禁言</button></div>"
	...
	if(th.id == "shelve"){
		layer.confirm("确定要删除该条举报信息吗?",{
			...
			$.ajax({
				url:"/gm/removeReportPlayer/"+arr[1],
				type:'get',
				dataType:"json",
				success:function(result){
					if(result){
